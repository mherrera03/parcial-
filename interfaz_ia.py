...
def responder(pregunta: str, df) -> str:
    ...
    if df is not None:
        ...
        if m:
            ...
            if match and pd.api.types.is_numeric_dtype(df[match]):
                ...
                return f"⚠️ No encontre el campo `{campo}` o no es numerico. Campos numericos disponibles: {', '.join(df.select_dtypes('number').columns.tolist())}"
        ...
        if m4:
            ...
            if match:
                ...
                return f"🏷️ Los valores unicos de **{match}** son:\n\n" + \
    ...
    respuestas_ml = {
        ...
        "sobreajuste": "⚠️ **Sobreajuste (Overfitting)** ocurre cuando el modelo aprende de memoria los datos de entrenamiento.\n\n"
...
def show():
    ...
    if st.session_state.messages:
        st.markdown("---")
        if st.button("🗑️ Limpiar conversacion", use_container_width=False):
            st.session_state.messages = []
            st.rerun()
